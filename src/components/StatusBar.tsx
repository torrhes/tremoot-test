      ...
      <div className="status-bar__right">
        ...
        {conflictCount > 0 && (
          <span className="status-pill status-pill--conflict">
            {t(pluralKey('sync.conflictCount', conflictCount, locale), {
              count: conflictCount
            })}
          </span>
        )}
          ...
          <span
            ...
            title={
              connectivity === 'offline'
                ? t('sync.offlinePendingTitle')
                : t('sync.pendingTitle')
            }
          >
            <span className="status-pill__spinner" />
            {t(pluralKey('sync.savedCount', pendingCount, locale), {
              count: pendingCount
            })}
          </span>
        )}
        ...
        <div className="status-bar__sync-tooltip">
          <button
            type="button"
            className="status-bar__sync-tooltip__trigger"
            aria-describedby="sync-tooltip-content"
            aria-label={t('sync.legendTitle')}
          >
            i
          </button>
          <div
            className="status-bar__sync-tooltip__content"
            role="tooltip"
            id="sync-tooltip-content"
          >
            <p>{t('sync.hint')}</p>
