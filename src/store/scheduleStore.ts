...
import { buildInitialAssignments, buildWorkShifts, buildUnassignedJobs, isLegacyShiftRow } from '../data/seed'
...
import type { Assignment, Conflict, ConnectivityState, Job, WorkShift } from '../types'
...
interface ScheduleState {
  ...
  init: () => Promise<void>
  assignJob: (jobId: string, shiftId: string, startMinute: number, endMinute: number) => Promise<void>
  moveAssignment: (assignmentId: string, shiftId: string, startMinute: number, endMinute: number) => Promise<void>
  unassign: (assignmentId: string) => Promise<void>
  resolveConflict: (conflictId: string, resolution: 'keep-local' | 'keep-remote') => Promise<void>
  setConnectivity: (state: ConnectivityState) => void
  addJob: (input: Omit<Job, 'id'>) => Promise<Job>
  updateJob: (jobId: string, changes: Partial<Omit<Job, 'id'>>) => Promise<void>
...
export const useScheduleStore = create<ScheduleState>((set, get) => ({
  ...
  init: async () => {
    ...
    subscribeToEngine((event) => {
      if (event.type === 'assignment-updated') {
        set((s) => ({
          assignments: { ...s.assignments, [event.assignment.id]: event.assignment },
        }))
      }
    ...
    window.addEventListener('offline', () => get().setConnectivity('offline'))
  ...
  assignJob: async (jobId, shiftId, startMinute, endMinute) => {
    ...
    set((s) => ({ assignments: { ...s.assignments, [assignment.id]: assignment } }))
  ...
  moveAssignment: async (assignmentId, shiftId, startMinute, endMinute) => {
    ...
    set((s) => ({ assignments: { ...s.assignments, [assignmentId]: updated } }))
  ...
  deleteJob: async (jobId) => {
    ...
    const assignment = Object.values(get().assignments).find((a) => a.jobId === jobId)
    if (assignment) {
  ...
  openAssignmentEditor: (assignmentId) => set({ editingAssignmentId: assignmentId }),
...
export function selectUnassignedJobs(state: ScheduleState): Job[] {
  const assignedJobIds = new Set(Object.values(state.assignments).map((a) => a.jobId))
